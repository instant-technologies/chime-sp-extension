...
export default class BotFrameworkChatPopupApplicationCustomizer extends BaseApplicationCustomizer<
  IBotFrameworkChatPopupApplicationCustomizerProperties
> {
  ...
  public onInit(): Promise<void> {
    ...
    console.log(
      LOG_SOURCE,
      `Configured sites: ${this.properties.directLineSecret}`
    );
    // clean up allowed sites
    const allowedSites =
      this.properties.allowedSites && this.properties.allowedSites.length
  ...
  private _renderPlaceHolders(): void {
    ...
    if (!this._bottomPlaceholder) {
      ...
      const elem: React.ReactElement = React.createElement(
        BotFrameworkChatPopupApplicationChat
      );
  ...
  private _onDispose(): void {
    console.log(
      "[ReactHeaderFooterApplicationCustomizer._onDispose] Disposed custom top and bottom placeholders."
    );
  }
}
